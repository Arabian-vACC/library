...
**Tag Denotation:**  
- Apply speed restrictions via TopSky tags using exact values or inequality markers (< / >).  
...
Note: While in-depth EuroScope knowledge is not required, familiarity with these features (especially tag management) significantly aids in radar controlling.  
